...
credential file, or literal. The transport receives a
`nullone_secret_provider.SecretValue` token at construction time via
`build_authenticated_transport`, which is the only function in this
adapter that accepts one. The transparent `SecretValue` wrapper makes an
accidental print impossible: `repr()`/`str()` render the fixed token
`<redacted>` and the dataclass field is excluded from autogenerated
repr output; the value is exposed only through the narrow `reveal()`
used by the transport to build the `Authorization` header.
"""
...
# Confirmed base URL (see module docstring). This adapter never reads it
# from any environment variable; `build_authenticated_transport` uses it
# as the default and it is only used for GET requests.
...
def build_authenticated_transport(
    *,
    ...
    """Construct the real GET-only transport from an injected `SecretValue`.
    ...
    `ConnectorUnauthorizedError` so an empty inherited variable surfaces
    as the same domain `BLOCKED` result as a missing one.
    """

    if not isinstance(token, SecretValue):
        raise TypeError("token must be a nullone_secret_provider.SecretValue")
    ...
    return UrllibAnalyticsTransport(base_url=base_url, token=token)
