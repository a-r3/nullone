...
def self_test() -> int:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        # 16b. A malformed notifier result carrying secret-like content in
        ...
        for index, bad_outcome in enumerate(leak_cases):
            result = run_analytics_workflow(
                make_trigger(external_occurrence_id=f"openclaw-occ-analytics-notifier-leak-{index}"),
                provider_factory=success_factory,
                notifier=lambda _r, _bad=bad_outcome: _bad,
                artifact_root=root / f"case16b-{index}",
                output_root=root / f"case16b-{index}" / "run-outcomes",
            )
