...
- **Release recommendation:** Do **not** proceed to controlled production activation (#37) yet. The reviewed prerequisite set for #37's controlled deployment/validation is #27 (domain outcomes/health), #28 (Morning Editorial runtime), #29 (Daily Analytics runtime/adapter), and #30 (failure alerts) — #27/#28/#29 are merged in Git but not deployed, and #30 is not yet complete. #30 is the next main engineering implementation item; production deployment of all four occurs within #37 itself, not as a separate pre-#37 stage. See "Release decision" below for exact sequencing.
...
- The reviewed prerequisite bundle for #37 is: **#27** (domain outcomes/health), **#28** (Morning Editorial runtime), **#29** (Daily Analytics runtime/adapter), and **#30** (failure alerts), plus any other reviewed M0 changes #37's scope requires. #27/#28/#29 are merged in Git but not deployed; #30 is not yet complete. These are the direct, evidenced defects from this proof window.
- Correct sequence from here:
  ...
  3. remaining required M0 engineering may proceed per existing roadmap — #31/#34 decision work may continue in parallel, followed by relevant Story/breaking-behavior implementation;
  4. once the reviewed prerequisite changes (#27/#28/#29/#30) are ready, issue #37 performs the controlled production deployment/preflight/live-validation boundary — it deploys the exact reviewed SHAs/components required and then observes genuine scheduled behavior before controlled production activation is considered complete.
- No separate, uncontrolled production deployment stage should occur before #37, except genuine emergency recovery of a concrete production failure under existing hotfix rules. No synthetic production cycles.
