...
execution failure, never reachability), secret-free metadata,
capability declaration, outcome. `invoke_adapter` runs exactly the
profile's transport — a failure propagates, never falls back.

- OpenCode transport accepts the model from the profile for every
  role; binary resolution, isolated session, explicit agent/model,
  `--format json`, exact `--dir`, no `--auto`/continuation,
  reviewed agents, timeouts, and fixed-string failures preserved.
- Claude transport remains the explicit rollback adapter (Morning
  cycle via `claude -p`, Story via `HaikuStoryWriter`), model from
  the profile with the reviewed `sonnet` default when unpinned.
  Claude supports `morning_editorial` + `story_writer` only; any
  other role x claude fails closed at resolve time.
