...
PR #109 merged the Packaging Contract V1 as contract + pure evaluator + prompt alignment, explicitly leaving runtime wiring as `PACKAGING_RUNTIME_WIRING=FOLLOW_UP_REQUIRED`. This branch implements that wiring, repo-only: `nullone-packaging-evaluator.py` (model assesses raw signals → deterministic receipt under `social/drafts/production/`, `verification`→`verification_status` mapped deterministically, model-authored decision fields rejected, idempotent/conflict-safe writes); `nullone_packaging_receipt.py` (hash-bound receipt authority shared by all gates); `nullone-packaging-render.py` (renders ONLY the receipt's format; carousel spec count must equal the recommendation; STORY delegated; SKIP renders nothing); `nullone-manifest.py build` now requires `--packaging-receipt` and refuses mismatch/SKIP/STORY before any caption/media work. The Draft Factory OpenCode agent boundary narrowed accordingly (direct renderer calls removed; evaluator + dispatcher only). Authority chain: signals → evaluator → receipt → render/manifest conform or BLOCKED; zero Zernio/Telegram on mismatch. Visual V3 remains out of scope. #111 remains provider-abstraction future; #37 remains OPEN. Natural production proof status is not upgraded by this merge. This PR is NOT deployed.
