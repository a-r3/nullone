...
- Weekly Strategy → `nullone-weekly-strategy-run.py` + `nullone-weekly-strategy` agent (no shell at all; writes scoped to the strategy report path + `MEMORY.md`; broader strategy-hypothesis files deliberately left for a follow-up that pins their exact path; 600s budget).
