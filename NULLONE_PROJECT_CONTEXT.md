...
### Remaining model-backed workflows migration — issue #112 (repo-level, NOT DEPLOYED)

Already merged: PR #110 (Morning → OpenCode, deployed with `NULLONE_EDITORIAL_PROVIDER=opencode`) and PR #113 (Story writer provider migration, activation pending). Temporary operational policy stands: all active model-backed workflows use OpenCode + Muse Spark (`opencode/muse-spark-1.3-contributor-free`) while Claude/Anthropic access is unavailable.

This PR migrates the last three agentTurn workflows to reviewed command-payload role wrappers (schedules untouched; live automations untouched):

- Draft Factory → `nullone-draft-factory-run.py` + `nullone-draft-factory` agent (DRAFT_FIRST preserved; reasoning in-agent, Zernio transport only via deterministic `nullone-draft-bridge.py`, Telegram preview/approval only via `openclaw message send` to the owner; scoped bash allowlist of exactly the reviewed renderer/manifest/bridge/send commands; sibling mutating message subcommands denied; 900s transport budget).
- Breaking Radar → `nullone-breaking-radar-run.py` + `nullone-breaking-radar` agent (DELTA_MONITORING_ONLY preserved; web research; writes scoped to breaking reports + `breaking-staging/`; handoff commits only via deterministic `nullone-breaking-scan.py`; no drafts/Telegram/publication; 600s budget).
- Weekly Strategy → `nullone-weekly-strategy-run.py` + `nullone-weekly-strategy` agent (no shell at all; writes scoped to the strategy report path + `MEMORY.md`; 600s budget). Explicit semantic note: `WEEKLY_LOW_RISK_HYPOTHESIS_WRITE=TEMPORARILY_DISABLED` — the reviewed prompt permits low-risk hypothesis updates but no canonical exact hypothesis file exists in the repository (governance docs like CONTENT_STRATEGY.md must not take agent writes), so the boundary denies them until a follow-up pins their exact path. This is a least-privilege narrowing during the urgent migration, not domain parity; the core report + MEMORY output stays operational.

Shared mechanics live in `nullone_opencode_role.py` (model resolution, deterministic argv, timeout/reachability failure mapping); capability boundaries stay per-role and were each proven live in disposable sandboxes (allowlisted write succeeds, off-scope write/secret-read/shell blocked; scan-helper execution succeeds). After this PR no covered workflow's repository configuration requires `anthropic/claude-sonnet-5` or `anthropic/claude-haiku-4-5` (removal from live automation `model:` fields happens at deployment, per-job, without schedule changes; rollout order Draft Factory → Radar → Weekly). Claude infrastructure stays installed as fallback. #112 stays OPEN until production activation/natural proofs complete; #111 stays OPEN for future role routing; #37 stays OPEN. This PR is NOT deployed.

### Deterministic packaging runtime wiring (repo-level, NOT DEPLOYED)

PR #109 merged the Packaging Contract V1 as contract + pure evaluator + prompt alignment, explicitly leaving runtime wiring as `PACKAGING_RUNTIME_WIRING=FOLLOW_UP_REQUIRED`. This branch implements that wiring, repo-only: `nullone-packaging-evaluator.py` (model assesses raw signals → ONE canonical receipt per candidate derived from candidate_id, no caller-chosen path; `verification`→`verification_status` mapped deterministically, model-authored decision fields rejected, idempotent/conflict-safe writes); `nullone_packaging_receipt.py` (hash-bound receipt authority shared by all gates, plus validated asset descriptors and render records); `nullone-packaging-render.py` (renders ONLY the receipt's format with ONLY its visual evidence: descriptor kind must equal the style mapping, feed source comes from validated evidence, carousel spec count must equal the recommendation, STORY delegated, SKIP renders nothing; every success writes a canonical render record binding outputs to the receipt hash); `nullone-manifest.py build` now requires `--packaging-receipt` AND `--render-record` and refuses mismatch/SKIP/STORY/substituted media before any caption/media work. The Draft Factory OpenCode agent boundary narrowed accordingly (direct renderer calls removed; evaluator + dispatcher only; receipt/render-record writes denied to the model — live-proven: request WRITTEN, receipt BLOCKED). Authority chain: signals → canonical receipt → validated evidence → render record → manifest conform or BLOCKED; zero Zernio/Telegram on mismatch. Visual V3 remains out of scope. #111 remains provider-abstraction future; #37 remains OPEN. Natural production proof status is not upgraded by this merge. This PR is NOT deployed.
